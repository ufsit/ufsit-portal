...
routes.get('/writeups/files/uploaded', async (req, res, next) => {
  ...
  list.map((value, index) => {
    const suffix = `.${value.name.slice((value.name.lastIndexOf('.') - 1 >>> 0) + 2)}`;
...
routes.post('/writeups/show_hide', async (req, res, next) => {
...
routes.post('/writeups/difficulty', async (req, res, next) => {
  if (util.account_has_admin(req.account)) { // require admin access
    try {
