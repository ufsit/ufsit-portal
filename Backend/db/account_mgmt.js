...
const account_mgmt_module = (function () {
  ...
  async function authenticate(login_data) {
    ...
    if (!(isEmail(login_data.email))) {
      ...
      if (email === 'left_blank@ufl.edu') { email = login_data.ufl_email; }
      ...
      const authenticated = verify_credentials(login_data.password, result.salt, result.hash);
  ...
  async function update_account(account_id, account_data) {
    if (account_data.password) {
      ...
      const newHash = hash_password(account_data.password, newSalt);
  ...
  function verify_credentials(given_pw, salt, stored_hash) {
    const test_hash = crypto.pbkdf2Sync(
      given_pw, salt,
      PBKDF2_NUM_ITERATIONS,
      PBKDF2_KEY_LEN,
      PBKDF2_ALGO,
  ...
  async function generate_session_token(account_id, ip_address, browser, time_to_expiration) {
    ...
    const expire_date = new Date(Date.now() + time_to_expiration);
